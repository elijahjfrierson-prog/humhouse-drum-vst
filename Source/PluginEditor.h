...
private:
    ...
    // v1.6.1-rc.24 — the FL-Studio-style chromatic piano roll was
    // removed. The user couldn't find it during normal use and the
    // step grid + DAW host MIDI capture (rc.24) cover both manual
    // editing and chromatic input. The arrangement-strip MidiDragHandle
    // (rc.18) still exports compositions to the host's piano roll
    // where they can be edited natively.
    ...
    // v1.6.1-rc.20 — PIANO ROLL + ONE-SHOT buttons removed in rc.24
    // (the FL-style chromatic piano-roll component was retired).
